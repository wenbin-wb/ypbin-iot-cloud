...
| 3 | `renew` | 周期性续约（默认 **10s**） | 幂等 | **响应里的 `revokedTenantIds` 是 self-fencing 判据**（见 §4） |
...
| 6 | `batchEpoch` | **一次拉全量**租户的 `epoch`（周期对账用，默认 5 分钟） | 只读 | 见 §5 —— 判据**只用 epoch** |
...
3. **旧节点 self-fencing（硬要求）**：旧节点一旦发现自己的租约不再有效，
   **必须立即断开本节点所有链路并停止采集**。
   - 依据有两条：① 本地的 `leaseExpireAt` 判断；② `renew` 响应里的 `revokedTenantIds`。
...
- 默认 **5 分钟**一次：`batchEpoch()` 一次拉全量 → 与本地比对 → **只对不一致的租户**拉全量台账；
...
| 响应体 | 一律 HTTP 200 + `R.code`；集合字段**永不为 null**（默认为空集合，有测试锁定） | 本契约 DTO |
