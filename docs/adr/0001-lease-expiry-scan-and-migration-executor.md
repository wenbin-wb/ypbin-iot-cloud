...
## 6. 附：P3 复核四轮的结论（2026-09-19，PR #1 已合并 `b52f549`）

| 轮次 | 结论 | 关键产物 |
|---|---|---|
| 1 | PASS（有条件）：失效检测与 epoch 语义正确，但**8 项必修** | 其中两项证伪了本文档：① `acquire` **单副本并发**就双主（不是「多副本才有问题」）；② `maxTenants` 缺省 NPE 而契约说「为空=不限」 |
| 2 | 7/8 闭合，但**修复自己引入 P1 回归** | `ypbin.lease.enabled=false` 让 business 起不来（属性注册被类级条件带走）→ 拆出无条件的 `LeasePropertiesAutoConfiguration`；回归门禁立刻又抓出第二处（控制器） |
| 3 | 6 项全闭合，但过滤器有 **2 个归一化绕过** | `//business//internal/…` 与 `/business/internal;a=b/…`（复验者删掉剥离名单后实测拿到内部数据）→ 判据改为与路由器归一化对齐，并自查出第 3 个（`..` 穿越） |
| 4 | **绕过已闭合** | 变异证明新用例真咬人；「去掉剥离名单」的做实实验不再拿到内部数据；另记录两条**容器假设**（反斜杠分隔符、多重解码）到过滤器注释 |

**结论**：本文档 §2.1 的核心判断（失效检测要独立执行者、M0b 要原子化）成立；但**「单副本不需要互斥」是错的**，已在 §2.1 改写。跨层的路径判据（§4「网关侧封堵内部端点」）必须与路由器归一化对齐，这一条已升级为通用教训（见母仓 `AGENTS.md` 教训三十）。
