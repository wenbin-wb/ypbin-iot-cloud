 ...
 *
 * ⚠️ **本阶段（M0a）不接 CI**：覆盖率快照属于第二批门禁（IOT-CLOUD-SPEC.md §9.1 第 8 项），
 *    M0b 接入。因此本仓**还没有** tools/generated/cloud-coverage.json —— 首次生成要等
 *    M0a 骨架长出真实代码之后（空快照只会把文档里的覆盖率变成永远为零的假数据）。
 *    在那之前 `--check` 会明确报错并告诉你先执行生成命令，这是有意的，不是坏掉。
 *
 * 为什么需要它：覆盖率数字一旦手工抄进 README / ROADMAP，同一份数据就会出现多个互不一致的版本，
 * 且每次改动都不会自动跟上。解法与配置元数据一样：**由构建产物生成一份，文档只引用它**。
 *
 * ⚠️ 为什么本文件**不做数值门禁**：覆盖率存在执行波动（同一提交、同一命令的两次全量运行
 * 实测差 0.04~0.78pp，竞争态分支的命中取决于队列满/空状态）。把波动的数字当门禁必然产生假失败。
 * 因此 `--check` 只校验**确定性的部分**：
 *   ① 快照必须存在且可解析；
 *   ② 快照里的模块集合必须与当前构建产物里的模块集合**完全一致**（新增/删除模块后忘记重新生成 → 红）。
 * 数值本身只在生成时刷新，供人阅读与对比。
 *
 * 用法：
 *   node tools/export-coverage.mjs                 # 生成（禁止静默缩小模块集合）
 *   node tools/export-coverage.mjs --allow-shrink  # 确实删了模块时重新生成基线
 *   node tools/export-coverage.mjs --check         # 校验模块集合未漂移（M0b 起进 CI）
 */
...
const outputPath = join(repoRoot, 'tools/generated/cloud-coverage.json');
...
const csvFiles = globSync('ypbin-iot-cloud-*/target/site/jacoco/jacoco.csv', { cwd: repoRoot }).sort();
...
if (checkOnly) {
  ...
  for (const entry of modules) {
    console.log(
      `  ${entry.module.padEnd(34)} 指令 ${entry.instructionPct}%  分支 ${entry.branchPct}%`
